...
def _liang(words):
    ...
    exceptions = {}
    for ex in exceps.split():
        exceptions[ex.replace('-', '')] = [0] + [ int(h == '-') for h in re.split(r"[a-z]", ex) ]
    ...
    syllables = []
    for word in words:
        syllables.append(len(hyphenate_word(word)))
...
def _legalipy(words):
    syllables = []
    for word in words:
        syllables.append(len(LegaliPy(word, getOnsets(word))))
...
def _sonoripy(words):
    syllables = []
    for word in words:
        syllables.append(len(SonoriPy(word)))
